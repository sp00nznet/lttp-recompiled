...
Real fix path: a minimal SIO multiplayer simulation that, on `SIOCNT.start = 1` writes, raises the SIO IRQ after a few cycles with SIOMULTI[0] = "no-slave error" pattern. Games that decide "no peers connected → run alone" off that IRQ will then advance. We already have the transfer-on-write-start simulation upstream (commit [`26522a1`](https://github.com/sp00nznet/gbarecomp/commit/26522a1)) but **LttP never writes start=1** — it waits for SD to read high *before* it'll even initiate, which the upstream code doesn't fake. For LttP specifically, the exact peer-table data needed to satisfy the -15 checksum check would need reverse-engineering of the Four Swords link-cable handshake. Multi-day project.
